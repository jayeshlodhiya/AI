...
// ===== API loaders =====
function normalizeAssistants(resp) {
  if (!resp) return [];
  if (Array.isArray(resp)) return resp;
  if (Array.isArray(resp.assistants)) return resp.assistants;
...
function seedIfEmpty(list) {
  ...
  // Safe fallback so the UI isn’t empty if API is down
  return [
    { id:'asst_001', name:'Manisha', company:'Awaaz AI', direction:'OUTBOUND', title:'Customer Engagement and Relationship Management', avatar_url:'https://placehold.co/88x88' },
    { id:'asst_002', name:'Arun',    company:'Awaaz AI', direction:'OUTBOUND', title:'Post-Call Follow-up Bot',                          avatar_url:'https://placehold.co/88x88' },
    { id:'asst_003', name:'Neha',    company:'Awaaz AI', direction:'INBOUND',  title:'Lead Qualification',                               avatar_url:'https://placehold.co/88x88' }
  ];
}
...
async function loadAssistants() {
  ...
  try {
    ...
    console.log("[ASSISTANTS][RAW]", raw);
    const list = seedIfEmpty(normalizeAssistants(raw));
    renderAssistants(list);
    if (statusText()) statusText().textContent = "Ready.";
  } catch (e) {
    console.error(e);
    if (statusText()) statusText().textContent = "Failed to load assistants (showing demo).";
    toast("Failed to load assistants (see console). Showing demo list.");
    renderAssistants(seedIfEmpty([]));
  }
}
...
function parseTranscript(raw) {
  ...
  if (Array.isArray(raw)) return raw; // server normalized
  ...
  for (let i = 0; i < 3; i++) {
    if (typeof v === "string") {
      ...
      try {
        ...
        if (typeof v === 'string') continue; // parsed to string, try again
        break;
      } catch { break; }
    } else {
      break;
...
const callModal = {
  ...
  init() {
    ...
    if (this.__bound) return; // prevent double binding
...
function ensureModalInit() {
  // Re-query & bind after the fragment is injected
  if (!callModal.el || !document.body.contains(callModal.el)) {
    callModal.init?.();
  }
}
...
function openCallDetail(record, opts = { autoplay: false }) {
  ...
  ensureModalInit();                // ensure modal nodes/listeners exist
  if (!callModal.el) {              // safety: if modal is missing in HTML
    console.warn('Modal root not found in DOM.');
    return;
  }
  ...
  if (record.recording_url) {
    ...
    audio.src =  record.recording_url; // adjust if needed
...
function renderCallHistoryFromApi(apiRows) {
  ...
  (apiRows || []).forEach((rec) => {
    ...
    tr._data = rec;
    tr.style.cursor = 'pointer'; // row clickable (opens modal)
  ...
  // ensure delegates are bound now that tbody exists
  wireCallHistoryDelegates();
}

// Delegated binder (bind once)
function wireCallHistoryDelegates() {
  const tbody = document.getElementById('callsTbody');
  ...
  tbody.addEventListener('click', (e) => {
    ...
    // If play button clicked → autoplay, else open without autoplay
    const playBtn = e.target.closest('[data-action="play-recording"]');
    openCallDetail(tr._data, { autoplay: !!playBtn });
  });
}

/* ----------------------------------------------------------------
   Page bootstrap that works with dynamic page injection
---------------------------------------------------------------- */
function wireStaticEventsOnce() {
  if (document.__aiCallWired) return;
  document.__aiCallWired = true;
  ...
  // Delegated: "Call" buttons on assistant cards
...
function tryInitPage() {
  const need = ["#assistantsList", "#assistantCardTemplate", "#callsTbody", "#callDetailModal"];
  if (!need.every(sel => document.querySelector(sel))) return;

  // Init once per injected fragment
  const root = document.querySelector("#assistantsList");
...
// Debounced scheduler to avoid storms from MutationObserver
let __aiCallInitScheduled = false;
function scheduleTryInitPage() {
  if (__aiCallInitScheduled) return;
  __aiCallInitScheduled = true;
  (window.requestAnimationFrame || setTimeout)(() => {
    __aiCallInitScheduled = false;
    tryInitPage();
  }, 16);
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", scheduleTryInitPage, { once: true });
} else {
  scheduleTryInitPage();
}

// Observe for dynamic injection (kept active; guarded by __inited flag)
if (!window.__aiCallObserver) {
  window.__aiCallObserver = new MutationObserver(() => scheduleTryInitPage());
  window.__aiCallObserver.observe(document.body, { childList: true, subtree: true });
}

/* ----------------------------------------------------------------
   Formatting helper used by transcriptPreNode
---------------------------------------------------------------- */
